...
module.exports = {
    ...
    fetchFavorites: function (res, user) {
        let path = "/favorites";
        checkFavorites(path, user, res);
    },

    addFavorite: function (res, user, favorites) {
        let path = "/favorites";
        addFavoriteToDb(res, path, favorites, user)
    }
};

function checkFavorites(path, user, res) {
    var MongoClient = require('mongodb').MongoClient;
    var url = "mongodb://localhost:27017/";
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("mydb");
        dbo.collection("cache").find({mypath: path, user: user}).toArray(function (err, result) {
...
function addFavoriteToDb(res, path, data, user) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        let dbo = db.db("mydb");
        dbo.collection("cache").find({mypath: path}).toArray(function (err, result) {
            let newEntry = {mypath: path, user: user, mydata: data};
            if (result.length > 0) {
                dbo.collection("cache").replaceOne({mypath: path, user: user}, newEntry);
                res.end("Favoris mis à jour.")
            } else {
                dbo.collection("cache").insertOne(newEntry, function (err, res) {
                    if (err) throw err;
                    console.log("Nouvelle donnée ajoutée à la base de donnée !");
                });
                res.end("Favoris créés.")
            }
        });
    });
}
